...
class Decoder(nn.Module):
    ...
    def forward(self, x, encoder_z, encoder_x):  # x: (batch_size,seq_len)
        first_attn_mask = (x == PAD_IDX).unsqueeze(1).expand(x.size()[0], x.size()[1], x.size()[1]).to(
            DEVICE)  # 目标序列的pad掩码
        first_attn_mask = first_attn_mask | torch.triu(torch.ones(x.size()[1], x.size()[1]),
                                                       diagonal=1).bool().unsqueeze(0).expand(x.size()[0], -1, -1).to(
            DEVICE)  # &目标序列的向后看掩码
        # 根据来源序列的pad掩码，遮盖decoder对其pad部分的注意力
        second_attn_mask = (encoder_x == PAD_IDX).unsqueeze(1).expand(encoder_x.size()[0], x.size()[1],
                                                                      encoder_x.size()[1]).to(
            DEVICE)  # (batch_size,target_len,src_len)
...
if __name__ == '__main__':
    ...
    enc = Encoder(vocab_size=len(de_vocab), emb_size=128, q_k_size=256, v_size=512, f_size=512, head=8, nblocks=3).to(
        DEVICE)
    enc_outputs = enc(enc_x_batch)
    print('encoder outputs:', enc_outputs.size())

    # Decoder编码,输出每个词对应下一个词的概率
    dec = Decoder(vocab_size=len(en_vocab), emb_size=128, q_k_size=256, v_size=512, f_size=512, head=8, nblocks=3).to(
        DEVICE)
